...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, display):
    ...
    while max(score) < score_to_win:
        ...
        pygame.event.pump()
        keys = pygame.key.get_pressed()
        if keys[K_q]: # press key q to quit the game
            return
        for event in pygame.event.get():
            if event.type == pygame.QUIT: # click on close X on the window to quite the game 
                return 
...
def init_game(left_ai, right_ai):
    table_size = (440, 280)
    paddle_size = (10, 70)
    ball_size = (15, 15)
    paddle_speed = 1
    ...
    clock_rate = 80
    turn_wait_rate = 3
    score_to_win = 11 # run more than 3 
    ...
    paddles[0].move_getter = left_ai.pong_ai
    ...
    # Uncomment following five lines of code to play the game again in a switching sides. 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-left', dest='left_ai', required = True)
    parser.add_argument('-right', dest='right_ai', required = True)

    args = parser.parse_args()

    left_ai = __import__(args.left_ai[:-3])
    right_ai = __import__(args.right_ai[:-3])

    init_game(left_ai, right_ai)

    # python PongAIvAI_v1.py -left chaser_ai.py -right student_ai.py 